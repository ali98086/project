    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center border-bottom mt-4 mb-3 pb-2">
                <a href="{{route('admin.ticket.priority.create')}}" class="btn btn-primary btn-sm">ایجاد اولویت جدید</a>
                ...
                <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center width-16-rem">نام اولویت</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{$ticketPriority->name}}</td>
                            <td class="text-center">

                                <input type="checkbox" id="{{$ticketPriority->id}}" onchange="changeStatus('{{ $ticketPriority->id }}')" data-url="{{route('admin.ticket.priority.status', $ticketPriority->id)}}" @if($ticketPriority->status===1) {{'checked'}} @endif/>
                            
                            </td>
                            <td class="text-center w-25">

                                <a href="{{route('admin.ticket.priority.edit', $ticketPriority->id)}}" class="btn btn-sm btn-primary"><i class="fa fa-edit" aria-hidden="true"></i> ویرایش</a>

                                <form action="{{route('admin.ticket.priority.destroy', $ticketPriority->id)}}" method="post" class="d-inline">
                                    @csrf
                                    @method('delete')
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status) {
                    ...
                    errorToast('امکان تغییر وضعیت اولویت وجود ندارد')
