    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center border-bottom mt-4 mb-3 pb-2">
                <a href="{{route('admin.content.faq.create')}}" class="btn btn-primary btn-sm">ایجاد پرسش جدید</a>
                ...
                <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center width-16-rem">پرسش</th>
                            <th class="text-center width-16-rem">خلاصه پاسخ</th>
                    ...
                    @foreach($faqs as $key=>$faq)
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="checkbox" id="{{$faq->id}}" onchange="changeStatusFaq('{{ $faq->id }}')" data-url="{{route('admin.content.faq.status', $faq->id)}}" @if($faq->status===1) {{'checked'}} @endif/>
                            </td>
                            <td class="text-center">
                                <a href="{{route('admin.content.faq.edit', $faq->id)}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                <form action="{{route('admin.content.faq.destroy', $faq->id)}}" class="d-inline" method="post">
                                    @csrf
                                    @method('delete')
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status) {
                    ...
                    errorToast('امکان تغییر وضعیت پرسش وجود ندارد')
