                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="checkbox" id="{{$comment->id}}" onchange="changeStatus('{{ $comment->id }}')" data-url="{{route('admin.market.comment.status', $comment->id)}}" @if($comment->status===1) {{'checked'}} @endif/>
                            </td>
                            <td class="text-center">

                                <a href="{{route('admin.market.comment.show', $comment->id)}}" class="btn btn-info btn-sm" type="submit"><i class="fa fa-eye"></i> نمایش</a>

                                @if($comment->approved == 1)
                                <a href="{{route('admin.market.comment.approved', $comment->id)}}" class="btn btn-warning btn-sm mr-1"><i class="fa fa-clock"></i> عدم تایید</a>
                                @else
                                <a href="{{route('admin.market.comment.approved', $comment->id)}}" class="btn btn-success btn-sm text-white mr-1"><i class="fa fa-check"></i> تایید</a>
                                @endif
